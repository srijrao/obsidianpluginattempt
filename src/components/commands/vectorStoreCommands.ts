...
class SemanticSearchModal extends Modal {
    constructor(app: App, private results: any[], private totalEmbeddings?: number) {
        super(app);
        // Add a specific class to this modal for better CSS targeting
    ...
    onOpen() {
        ...
        // Clear any default modal styling
        contentEl.empty();
        contentEl.style.cssText = '';
        
        // Add the same CSS class as the codeblock
        contentEl.addClass('semantic-search-container');
        
        // Create header matching the codeblock style exactly
        ...
        // Clear any default span styling
        ...
        // Create results container matching the codeblock
        ...
        this.results.forEach((result, index) => {
            ...
            // Result header with similarity score and ranking (same as codeblock)
            ...
            // Add file link if available (same as codeblock)
            if (result.filePath) {
                ...
                linkEl.addEventListener('click', (e) => {
                    e.preventDefault();
                    // Close modal and open the file
                    this.close();
                    this.app.workspace.openLinkText(result.filePath, '', false);
                });
            }

            // Result content (same as codeblock)
            const contentEl = resultEl.createDiv({ 
                cls: 'semantic-search-content'
            });
            contentEl.style.cssText = 'line-height: 1.6 !important; color: var(--text-normal) !important; font-size: 0.9em !important; font-family: var(--font-interface) !important;';
