...
export function createChatUI(app: App, contentEl: HTMLElement): ChatUIElements {
  ...
  function createIndicator(options: { className: string }): HTMLElement {
  ...
  fadedHelp.setText(
    "Tip: Type /help or press Ctrl+Shift+H for chat commands and shortcuts. Use Ctrl+Shift+X to clear chat and Ctrl+Shift+C to copy."
  );
  ...
  const secondaryTopButtons = [
    {
      key: "referenceNoteButton",
      text: "📝",
      ariaLabel: "Toggle referencing current note",
      addClass: "ai-chat-reference-button",
    },
    {
      key: "obsidianLinksButton",
      text: "🔗",
      ariaLabel: "Toggle Obsidian links",
      addClass: "ai-chat-obsidian-links-button",
    },
    {
      key: "contextNotesButton",
      text: "📚",
      ariaLabel: "Toggle context notes",
      addClass: "ai-chat-context-notes-button",
    },
  ];
  ...
  const referenceNoteIndicator = createIndicator({
    className: "ai-reference-note-indicator",
  });
  const obsidianLinksIndicator = createIndicator({
    className: "ai-obsidian-links-indicator",
  });
  const contextNotesIndicator = createIndicator({
    className: "ai-context-notes-indicator",
  });

  // Model Name Display Container (separate line from buttons)
  const modelDisplayContainer = contentEl.createDiv(
    "ai-model-display-container"
  );
  modelDisplayContainer.style.textAlign = "center";
  modelDisplayContainer.style.margin = "0.5em 0";
  modelDisplayContainer.style.borderBottom =
  ...
  const toolContinuationContainer = contentEl.createDiv(
    "ai-tool-continuation-container"
  );
  ...
  addCurrentNoteButton.setAttribute(
    "aria-label",
