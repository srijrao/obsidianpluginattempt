...
export class ChatUIManager implements IChatUIManager {
  ...
  updateContextNotesIndicator(isEnabled: boolean, contextNotes?: string): void {
    ...
    if (isEnabled && notesList.length > 0) {
      const notesText =
        notesList.length === 1 ? notesList[0] : `${notesList.length} notes`;
  ...
  updateButtonStates(): void {
    ...
    this.updateReferenceNoteIndicator(
      this.uiState.referenceNoteEnabled,
      this.uiState.referenceNoteFile
    );
    ...
    if (this.plugin?.settings) {
      ...
      this.updateObsidianLinksButtonState(
        this.plugin.settings.enableObsidianLinks
      );
      this.updateContextNotesButtonState(
        this.plugin.settings.enableContextNotes
      );
    }
  }
  ...
  private updateTypingIndicator(): void {
    ...
    let indicator = this.uiElements.messagesContainer.querySelector(
      ".typing-indicator"
    ) as HTMLElement;
    ...
    if (this.uiState.isTyping) {
      if (!indicator) {
        ...
        indicator.innerHTML =
          '<span>AI is typing</span><div class="typing-dots"><span></span><span></span><span></span></div>';
